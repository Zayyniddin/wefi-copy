				...
				<div class="bg-white h-full text-black p-6 rounded-lg shadow">
					...
					<div class="text-sm mt-4">
						...
						<div class="flex items-center justify-between">
							...
							<p class="text-xl font-bold">
								{{ formatNumber(data.women_sum) }} mln
							</p>
						</div>
					</div>
				</div>
...
const pieOption = computed(() => ({
	...
	title: {
		text: `{value|${formatNumber(
			(props.data?.women_sum ?? 0) + (props.data?.men_sum ?? 0)
		)}}\n{label|Billion}`,
		left: 'center',
		top: 'center',
		textStyle: {
	...
	series: [
		{
			...
			data: [
				{
					value: Math.round(props.data?.men_sum ?? 0),
					name: 'Men',
				},
				{ value: Math.round(props.data?.women_sum ?? 0) ?? 0, name: 'Women' },
			],
		},
	],
}))
...
const businessTypeChart = computed(() =>
  generate({
    seriesData: [
      {
        ...
        data: [
          {
            value: props.data?.legal_women_percent ?? 0,
            name: 'Legal en.',
          },
          {
            value: props.data?.individual_women_percent ?? 0,
            name: 'Individual',
          },
        ],
      ...
      },
      {
        ...
        data: [
          {
            value: props.data?.legal_men_percent ?? 0,
            name: 'Legal en.',
          },
          {
            value: props.data?.individual_men_percent ?? 0,
            name: 'Individual',
          },
        ],
        style: { color: '#018E20' },
    ...
    totalsPercent: [
      props.data?.individual_percent ?? 0,
      props.data?.legal_percent ?? 0,
    ],
  })
)
    ...
    seriesData: [
      {
        name: 'Women',
        data: [
          { value: props.data?.micro_women_percent ?? 0, name: 'Micro' },
          { value: props.data?.small_women_percent ?? 0, name: 'Small' },
          { value: props.data?.medium_women_percent ?? 0, name: 'Medium' },
        ],
        style: { color: '#4A3AFF', borderRadius: [8, 0, 0, 8] },
      },
      {
        name: 'Men',
        data: [
          { value: props.data?.micro_men_percent ?? 0, name: 'Micro' },
          { value: props.data?.small_men_percent ?? 0, name: 'Small' },
          { value: props.data?.medium_men_percent ?? 0, name: 'Medium' },
        ],
        style: { color: '#018E20' },
      },
    ],
    totalsPercent: [
      props.data?.medium_percent ?? 0,
      props.data?.small_percent ?? 0,
      props.data?.micro_percent ?? 0,
    ],
  })
)
